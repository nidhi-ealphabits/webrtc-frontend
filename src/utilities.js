export const TRIANGULATION = [
  ...
  export function displayIrisPosition(predictions,ctx) {
    ctx.strokeStyle = "red";
    if(predictions.length > 0) {
        predictions.forEach(prediction => {
           const keyPoints = prediction.keypoints;
  ...
  let eyesClosed = 0;
  export function detectBlinkingEyes(predictions) {
      // ctx.fillStyle = "red";
      
      if(predictions.length > 0) {
          predictions.forEach(prediction => {
            
              const rightEyeUpper0 = prediction.keypoints[159];
              const rightEyeLower0 = prediction.keypoints[145];
              const leftEyeUpper0 = prediction.keypoints[386];
              const leftEyeLower0 = prediction.keypoints[374];
              // console.log(rightEyeLower0)
              ...
              let rightEyeCenterPointDistance = Math.abs((rightEyeUpper0.y-rightEyeLower0.y));
              let leftEyeCenterPointDistance = Math.abs((leftEyeUpper0.y-leftEyeLower0.y));
  
  
              // console.log("---------------",rightEyeCenterPointDistance);
              /* console.log("---------------");
